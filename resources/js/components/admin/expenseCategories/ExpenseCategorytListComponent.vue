  ...
  <div class="col-12">
    <div class="db-card">
      <div class="db-card-header border-none">
        <h3 class="db-card-title">Category</h3>
        <div class="db-card-filter">
          ...
          <ExpenseCategoryCreateComponent :props="props" v-if="permissionChecker('products_create')"/>
        </div>
      </div>
      ...
      <div class="table-filter-div">
        <form class="p-4 sm:p-5 mb-5" @submit.prevent="search">
          <div class="row">
            <div class="col-12 sm:col-12 md:col-12 xl:col-12">
        ...
        <table class="db-table stripe" id="print">
          ...
          <tr class="db-table-head-tr">
            ...
            <th class="db-table-head-th hidden-print"
                v-if="permissionChecker('products_edit') || permissionChecker('products_delete')">
              {{ $t('label.action') }}
            </th>
          </tr>
          </thead>
          <tbody class="db-table-body" v-if="items.length > 0">
          <tr class="db-table-body-tr" v-for="product in items" :key="product">
...
export default {
  name: "ExpenseCategorytListComponent",
  components: {
    TableLimitComponent,
    PaginationSMBox,
    PaginationBox,
  ...
  mounted() {
    this.list();
    this.$store.dispatch('expenseCategory/lists', this.searchProps);
  },
  methods: {
    permissionChecker(e) {
    ...
    edit: function (product) {
      ...
      this.$store.dispatch('expenseCategory/edit', product.id);
      this.loading.isActive = false;
    ...
    xls: function () {
      this.loading.isActive = true;
      this.$store.dispatch("expenseCategory/export", this.props.search).then((res) => {
        this.loading.isActive = false;

<template>
    ...
    <div class="md:w-[calc(100%-340px)] lg:w-[calc(100%-320px)] xl:w-[calc(100%-377px)]">
        <form class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4" @submit.prevent="search">
            <div
                class="col-span-2 lg:col-span-1 flex items-center h-10 px-3 rounded-md border border-[#EFF0F6] bg-white">
            ...
            <div class="db-field">
                <vue-select v-model="checkoutProps.form.brand" class="db-field-control appearance-none cursor-pointer"
                    id="customer" :options="brands" label-by="name" value-by="id" :closeOnSelect="true"
                    :searchable="true" :clearOnClose="true" :placeholder="$t('label.select_brand')"
    ...
    <div
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-1 w-20 p-1 rounded-full bg-[#F7F7FC]">
                            ...
                            <input v-on:keypress="onlyNumber($event)" v-on:keyup="quantityUp(index, cart, $event)"
                                type="number" :value="cart.quantity" class="text-center w-full h-5 text-sm font-medium">
        ...
        <div class="p-4">
            ...
            <div class="sm:col-span-3 mb-4">
                <label for="payment_method" class="db-field-title after:hidden">Select Payment method</label>
                <div class="mt-2">
            ...
            <div class="sm:col-span-3 mb-4">
                <label for="payment_method" class="db-field-title after:hidden">Select Purchase method</label>
                <div class="mt-2">
                    <div class="flex h-[38px] mb-4">
                        <div class="db-field-down-arrow">
                        ...
                        <input v-if="showAmountField" v-model="amountPaid" type="text"
                            v-on:keypress="floatNumber($event)" placeholder="Initial amount paid"
                            class="w-full h-full border-t border-b px-3 border-[#EFF0F6]">
                        <button v-if="showAmountField" @click.prevent="applyAmountPaid" type="submit"
                            class="flex-shrink-0 w-16 h-full text-sm font-medium font-client capitalize ltr:rounded-tr ltr:rounded-br rtl:rounded-tl rtl:rounded-bl text-white bg-[#008BBA]">
                            {{ $t('button.apply') }}
                        </button>
                    </div>
        ...
        currencyFormat(initialPayment,
            ...
            setting.site_currency_position)
                ...
                <li v-if="showAmountField" class="flex items-center justify-between">
                    ...
                    <span class="text-sm font-medium font-client capitalize leading-6 text-[#2E2F38]">
                        {{
        currencyFormat((subtotal + totalTax + deliveryPrice - initialPayment) - posDiscount,
            setting.site_digit_after_decimal_point, setting.site_default_currency_symbol,
            setting.site_currency_position)
    }}
                    </span>
...
export default {
    ...
    computed: {
        ...
        initialPayment: function () {
            return this.$store.getters['posCart/initialPayment'];
        },
    ...
    mounted() {
        ...
        this.productBrands();
        this.productList();
        this.$store.dispatch('posCart/addDeliveryCharge', 0).then(res => {
        }).catch();
        this.$store.dispatch('posCart/addInitialAmount', 0).then(res => {
        }).catch();
        try {
            this.customerList();
    ...
    methods: {
        ...
        handlePurchaseMethodChange() {
            ...
            // Determine whether to show the initial amount input field based on the selected purchase method
            this.showAmountField = this.purchaseMethod === "20" || this.purchaseMethod === "25";
        },
        applyAmountPaid() {
            if (this.amountPaid) {
                const amount = parseFloat(this.amountPaid);
                // console.log('hello' + this.amountPaid);
                this.$store.dispatch('posCart/addInitialAmount', amount).then(res => {
                }).catch();
            }
        },
        orderSubmit: function () {
            ...
            this.form = {
                ...
                subtotal: this.subtotal + this.deliveryPrice,
                discount: parseFloat(this.posCartDiscount),
                tax: this.totalTax,
                total: this.subtotal + this.totalTax + this.deliveryPrice,
                order_type: parseInt(this.purchaseMethod),
                source: sourceEnum.POS,
                payment_method: parseInt(this.paymentMethod),
                products: JSON.stringify(this.posCartProducts),
                shipping_charge: this.deliveryPrice,
                initial_amount: parseInt(this.amountPaid),
            }
